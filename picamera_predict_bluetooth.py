...
class Object_Recognition:
    ...
    def __init__(self, config, weight):
        ...
        self.rpi_bt_port = 1 # RPI's bluetooth port
        self.bt_address = "98:D3:A1:FD:44:A7"    # MAC address of target
        self.rpi_bt = bluetooth.BluetoothSocket(bluetooth.RFCOMM)
        self.rpi_bt.connect((self.bt_address, self.rpi_bt_port))
    ...
    def motion_stop(self):
        ...
        for video in camera.capture_continuous(rawCapture, format=self.camera_format, use_video_port=True):
            ...
            if wait_count > 10:
                ...
                camera.close()
                cv2.destroyAllWindows()
                self.rpi_bt.send(str(1)) # alert arduino to activate sensors for detection
                return self.state_function()
    ...
    def move_trash(self):
        ...
        """ wait for ready signal from arduino before restarting cycle """
        ready_signal = ""
        while ready_signal.__contains__("1") == False:
            print(ready_signal)
            ready_signal = self.rpi_bt.recv(10).decode("utf-8", "ignore")
            print(ready_signal)
